...
SCORED_FIELDS = ["auth_primary", "gate", "mcp", "composio_toolkit", "buildability"]
...
def _field_values(rec: dict) -> dict:
    return {
        "auth_primary": _auth_primary(rec["auth_schemes"]),
        "gate": rec["gate"],
        "mcp": rec["mcp"],
        "composio_toolkit": rec["composio_toolkit"],
        "buildability": rec["buildability"],
    }


def _gold_values(g: dict) -> dict:
    return {
        "auth_primary": _auth_primary(g.get("auth_schemes", ["UNKNOWN"])),
        "gate": g.get("gate", "unknown"),
        "mcp": g.get("mcp", "unknown"),
...
def _score_pass(records: dict[str, dict], gold: list[dict], pop: int) -> dict:
    ...
    for g in gold:
        ...
        agent_unknown = rv["auth_primary"] == "UNKNOWN" or rv["gate"] == "unknown"
        gold_unknown = gv["auth_primary"] == "UNKNOWN" or gv["gate"] == "unknown"
        if agent_unknown:
            if gold_unknown:
                good_unknown += 1
            else:
...
def score(run: str = "latest") -> dict:
    ...
    out = {
        ...
        "method_note": (
            ...
            "model and method than the Gemini extractor). This measures agreement between two "
            "independent processes over the same primary sources — a real check, but not a "
            "substitute for multi-person review, which would tighten both the labels and the "
            "interval. The composio_toolkit field is deterministic (from the catalog diff), so "
            "it is expected to match trivially and is not a model judgment."
        ),
    }
    (rd / "accuracy.json").write_text(json.dumps(out, indent=2))
